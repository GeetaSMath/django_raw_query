...
def register_user(request):
    ...
    if request.method == "POST":
        with connection.cursor() as cursor:
            cursor.execute(
                "INSERT INTO operations_user(first_name, last_name, username, password) VALUES (%s, %s, %s, %s)",
                (first_name, last_name, username, password))
            cursor.execute("select * from operations_user WHERE first_name=%s", [first_name])
...
def retrieve_users(request):
    if request.method == "GET":
        ...
        cursor.execute("select * from operations_user")
...
def update_user(request):
    ...
    if request.method == "PUT":
        with connection.cursor() as cursor:
            cursor.execute(
                "UPDATE operations_user SET first_name=%s, last_name=%s, username=%s, password=%s where id=%s",
...
def delete_user(request):
    ...
    if request.method == "DELETE":
        with connection.cursor() as cursor:
            cursor.execute("DELETE FROM operations_user WHERE id=%s", [user_id])
